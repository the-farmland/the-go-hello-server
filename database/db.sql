...
-- Add food_and_drink column to locations table
ALTER TABLE public.locations ADD COLUMN IF NOT EXISTS food_and_drink jsonb NULL;
...
BEGIN
  ...
  UNION ALL
    ...
    WHERE (p.pin ->> 'name') ILIKE '%' || search_query || '%'
       OR (p.pin ->> 'info') ILIKE '%' || search_query || '%'
  UNION ALL
    SELECT l.id, 'food_and_drink', p.idx, p.pin ->> 'name', p.pin ->> 'id'
    FROM locations l
    CROSS JOIN LATERAL jsonb_array_elements(COALESCE(l.food_and_drink, '[]'::jsonb)) WITH ORDINALITY AS p(pin, idx)
    WHERE (p.pin ->> 'name') ILIKE '%' || search_query || '%'
       OR (p.pin ->> 'info') ILIKE '%' || search_query || '%'
  ),
  sub_match AS (
